...
- Treat V2-A as a local/demo-day Meridian V2 extension; V2-B/GARP Authority Runway is a local G1-G5 authority runway; V2-B Foreman/Auth local proof cockpit is local/pre-deployment guide/explainer truth plus AUTH-5 deployed Vercel/Auth0 demo proof only; V2-C is dashboard-local presentation/choreography/reliability truth only; V2-B Demo UI Clarity is dashboard-local presenter hierarchy truth only; V2-D++ is dashboard-local embodied proof theater truth only; V2-E is dashboard-local visibility/proof hierarchy truth only; V2-F is dashboard-local presentation/voice proof truth only.
- Treat eval warm-tabs, phone smoke, mobile/judge device proof, full authority choreography proof, Walk-mode MP4 proof, clean logout proof, deploy hook cleanup proof, production/legal/live-city/OpenFGA/CIBA/notification/public-portal/model/API/audio/root-ForensicChain claims, and final V2-B closeout as HOLD until Tim supplies proof.
...
- Treat V2-C work as dashboard-local presentation/choreography/reliability only: Mission presentation skin, Mission Rail, Fictional Demo Permit #4471, HOLD Wall, Absence Lens presentation overlay, Decision Counter, Demo Audit Wall, Foreman audio identity, Disclosure Receipt, Doctrine Card, reliability panel/runbook/checklists, SyncPill, approval pulse, and vibration fallback over existing proof. Treat V2-B Demo UI Clarity work as presenter hierarchy only: Presenter Cockpit default, compact demo anchor, current decision/HOLD focal card, six-stage process rail, grouped proof tools, secondary decision summary, and compact safety card over existing proof. Neither claims shared contract widening, root governance/forensic/authority/live absence/root skins changes, Auth0/Vercel/env/package/deploy/config/secret/security changes, production city behavior, legal/TPIA/TRAIGA sufficiency, official Fort Worth workflow, public portal behavior, live OpenFGA, CIBA, delivered notifications, model/API-backed Foreman, external voice service, Whisper/audio upload/transcription, or manual proof completion.
- Treat V2-D++ work as dashboard-local demo/proof theater only: Guided Mission, Foreman Autonomous deterministic scripted conduct, embodied avatar state, Proof Spotlight, Absence Shadow Map, Authority Handoff Theater, Judge Touchboard, Evidence Navigator, Civic Twin Diorama, Forensic Receipt Ribbon, Mission Run Receipt Panel, Physical Mode, Rehearsal Certification, Failure Injection, and reliability guards over existing dashboard proof. It does not claim shared contract widening, production city infrastructure, official Fort Worth workflow, legal/TPIA/TRAIGA sufficiency, public portal behavior, live OpenFGA, CIBA, delivered notifications, live LLM/model/API-backed Foreman, browser-exposed key behavior, external voice service, Whisper/audio upload/transcription, root ForensicChain writes, legal audit trail behavior, live GIS/Accela/city-record behavior, mobile/judge-device proof, or manual/global HOLD completion.
- Treat V2-E work as dashboard-local visibility/proof hierarchy only: hard mission-surface visibility, product-facing default state, six-act guided reveal, completion Review Mode, proof card polish, review-only Failure Injection surfacing, guided progression repair, and duplicate-key warning repair over existing dashboard proof. It does not claim shared contract widening, runtime behavior, production city infrastructure, official Fort Worth workflow, legal/TPIA/TRAIGA sufficiency, public portal behavior, live OpenFGA, CIBA, delivered notifications, live LLM/model/API-backed Foreman, external voice service, Whisper/audio upload/transcription, root ForensicChain writes, live Constellation broker proof, mobile/judge-device proof, or manual/global HOLD completion.
- Treat V2-F work as dashboard-local presentation/voice proof only: one Foreman line, one focal card, one Next/Finish button, six-act mission narration, existing B6 browser-native voice/fallback, Act 4 3000ms silence, and Review Mode cockpit restoration over existing dashboard proof. It does not claim shared contract widening, runtime behavior, production city infrastructure, official Fort Worth workflow, legal/TPIA/TRAIGA sufficiency, public portal behavior, live OpenFGA, CIBA, delivered notifications, live LLM/model/API-backed Foreman, external voice service, Whisper/audio upload/transcription, MediaRecorder/getUserMedia, root ForensicChain writes, live city integration, live GIS/Accela automation, deployed proof completion, screenshot proof completion, mobile/judge-device proof, or manual/global HOLD completion.
...
- When touching V2-B Demo UI Clarity, sync `docs/closeouts/MERIDIAN_V2B_DEMO_UI_CLARITY_CLOSEOUT.md`, `dashboard/README.md`, `docs/UI_INDEX.md`, and current-truth front doors. Update `MIGRATIONS.md` only if a real shared/root contract changes.
...
- When touching the V2-D++ Embodied Foreman Civic Proof Theater finish lane, sync `docs/specs/MERIDIAN_V2D_EMBODIED_FOREMAN_CIVIC_PROOF_THEATER.md`, `docs/closeouts/MERIDIAN_V2D_EMBODIED_FOREMAN_CIVIC_PROOF_THEATER_CLOSEOUT.md`, `dashboard/README.md`, and current-truth front doors without editing runtime, dashboard source, tests, package files, auth/config/deploy/env/security surfaces, dashboard scenarios, dashboard authority endpoint, root `src/**`, or historical V1/V2 closeouts. Update `MIGRATIONS.md` only if a real shared/root contract changes.
- When touching the V2-E Visibility Cleanup finish lane, sync `docs/specs/MERIDIAN_V2E_VISIBILITY_CLEANUP.md`, `docs/closeouts/MERIDIAN_V2E_VISIBILITY_CLEANUP_CLOSEOUT.md`, `dashboard/README.md`, and current-truth front doors without editing runtime, dashboard source, tests, package files, auth/config/deploy/env/security surfaces, dashboard scenarios, dashboard authority endpoint, root `src/**`, governance engines, authority logic, ForensicChain logic, or data contracts. Update `MIGRATIONS.md` only if a real shared/root contract changes.
- When touching the V2-F Foreman Simplification + Voice finish lane, sync `docs/specs/MERIDIAN_V2F_FOREMAN_SIMPLIFICATION_VOICE.md`, `docs/closeouts/MERIDIAN_V2F_FOREMAN_SIMPLIFICATION_VOICE_CLOSEOUT.md`, `dashboard/README.md`, and current-truth front doors without editing runtime, dashboard source, tests, package files, auth/config/deploy/env/security surfaces, dashboard scenarios, dashboard authority endpoint, root `src/**`, governance engines, authority logic, ForensicChain logic, data contracts, or `MIGRATIONS.md` unless a real shared/root contract changes. Emergency P0 dashboard edits require browser-proof evidence of a real demo blocker.
