...
- `dashboard/api/` (dashboard-local shared authority endpoint behavior for deploy-prep proof)
...
- V2-C ships dashboard-local presentation/choreography/reliability only. The V2-B Demo UI Clarity pass ships presenter hierarchy only over that dashboard proof. V2-D++ ships dashboard-local demo/proof theater only over that dashboard proof. V2-E ships dashboard-local visibility/proof hierarchy only. V2-F ships dashboard-local presentation/voice proof only. None ships new runtime behavior, root/shared contract widening, root governance/forensic/authority/live absence/root skins changes, Auth0/Vercel/env/package/deploy/config/secret/security changes, production city behavior, legal/TPIA/TRAIGA sufficiency, official Fort Worth workflow, public portal behavior, live OpenFGA, CIBA, delivered notifications, model/API-backed Foreman, external voice service, Whisper/audio upload/transcription, MediaRecorder/getUserMedia, root ForensicChain writes, or final V2-B closeout.
