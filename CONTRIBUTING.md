...
- Treat V2-A as a local/demo-day Meridian V2 extension; V2-B/GARP Authority Runway is a local G1-G5 authority runway; V2-B Foreman/Auth local proof cockpit is local/pre-deployment guide/explainer truth only.
...
- Treat V2-B Foreman/Auth work as local/pre-deployment dashboard proof cockpit truth only: deterministic Foreman context, offline source-bounded narration, authority-aware narration, guided event binding, Gold modes, browser-native voice fallback, deterministic avatar state, dashboard-local Auth0 role-session mapping, and shared local `/api/authority-requests` endpoint behavior. It does not claim deployed URL proof, live Auth0 callback proof, model/API calls, external voice service, Whisper/audio upload/transcription, or final V2-B closeout.
